...
export function SiteHeader({ user, onLogout, compact = false }) {
  ...
  // Inline styles for immediate mobile responsive effect
  const headerStyle = {
    display: 'flex',
    alignItems: 'center',
  ...
  return (
    <header className="site-app-header" style={headerStyle}>
      <div className="header-container" style={containerStyle}>
        ...
        {/* Desktop Navigation */}
        {!compact && (
          <nav className="site-app-nav" style={desktopNavStyle} aria-label="Main">
        ...
        {/* Desktop Actions */}
        <div className="site-app-header-actions" style={desktopActionsStyle}>
          <a href={SITE_CONFIG.PHONE_HREF} className="header-phone">
            <Phone size={15} /> 
            <span style={{ display: window.innerWidth <= 767 ? 'none' : 'inline' }}>
              {SITE_CONFIG.PHONE}
            </span>
          </a>
          {user ? (
